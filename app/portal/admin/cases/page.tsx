...
export default function CasesPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    // Try to fetch with next_hearing_date; fall back without it if migrations haven't run yet
    const [casesResult, { data: clientsData }] = await Promise.all([
      supabase
        .from("cases")
        .select("id, title, status, practice_area, created_at, client_id, next_hearing_date, profiles(full_name)")
    ...
    if (casesResult.error) {
      // Column may not exist yet — retry without next_hearing_date
      const { data: fallbackData, error: fallbackError } = await supabase
        .from("cases")
        .select("id, title, status, practice_area, created_at, client_id, profiles(full_name)")
        .order("created_at", { ascending: false });
      if (fallbackError) {
        console.error("Cases fetch error:", fallbackError.message);
      }
      // Map to Case type with null next_hearing_date
      const mapped = (fallbackData ?? []).map((c: any) => ({ ...c, next_hearing_date: null }));
      setCases(mapped as any);
    } else {
  ...
  const sorted = [...filtered].sort((a, b) => {
    switch (sortKey) {
      case "hearing": {
        ...
        // Past hearings go to end; soonest first
        const now = Date.now();
  ...
  return (
    <div className="min-h-full px-8 py-10">
      ...
      <div className="border border-[#1e1e1e]">
        ...
        {loading ? (
          <div className="px-6 py-10 text-center text-sm text-silver-dim">Loading…</div>
        ) : sorted.length === 0 ? (
          ...
          sorted.map((c) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-xs text-silver-dim">{(c.profiles as any)?.full_name ?? "—"}</div>
                <div className="text-xs text-silver-dim">{c.practice_area}</div>
                <div className="flex items-center gap-1.5">
                  {hearing.urgent && <IconAlertTriangle size={11} className="text-yellow-400 shrink-0" />}
                  <span className={`text-xs ${hearing.urgent ? "text-yellow-400" : "text-silver-dim"}`}>
                    {hearing.label}
                  </span>
                </div>
                <div>
                  <span className={`inline-block border px-2 py-0.5 text-[10px] uppercase tracking-widest ${statusColors[c.status] ?? statusColors.pending}`}>
                    {c.status}
                  </span>
                </div>
              </Link>
      ...
      <AnimatePresence>
        {showModal && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm" onClick={() => setShowModal(false)} />
            <motion.div initial={{ opacity: 0, scale: 0.96, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96, y: 12 }} transition={{ duration: 0.2 }} className="fixed inset-x-0 top-1/2 z-50 mx-auto w-full max-w-md -translate-y-1/2 border border-[#2e2e2e] bg-[#111111] p-8 shadow-2xl">
              <div className="mb-6 flex items-center justify-between">
                ...
                <button onClick={() => setShowModal(false)} className="text-silver-dim hover:text-silver"><IconX size={18} /></button>
              </div>
              <form onSubmit={handleCreateCase} className="space-y-4">
                <div>
                  <label className="mb-1.5 block text-[10px] uppercase tracking-widest text-silver-dim">Case Title</label>
                  <input type="text" required placeholder="e.g. Property Dispute – Plot 421" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className={inputClasses} />
                </div>
                <div>
                  <label className="mb-1.5 block text-[10px] uppercase tracking-widest text-silver-dim">Client</label>
                  <select required value={form.client_id} onChange={(e) => setForm({ ...form, client_id: e.target.value })} className={inputClasses}>
                ...
                <div>
                  ...
                  <select required value={form.practice_area} onChange={(e) => setForm({ ...form, practice_area: e.target.value })} className={inputClasses}>
                    <option value="" disabled>Select area</option>
                    {PRACTICE_AREAS.map((a) => <option key={a} value={a}>{a}</option>)}
                  </select>
                </div>
                <div>
                  <label className="mb-1.5 block text-[10px] uppercase tracking-widest text-silver-dim">Status</label>
                ...
                <div>
                  ...
                  <textarea rows={3} placeholder="Brief description…" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className={`${inputClasses} resize-none`} />
                </div>
                {error && <div className="border border-red-900/40 bg-red-950/20 px-4 py-3 text-xs text-red-400">{error}</div>}
                <button type="submit" disabled={creating} className="w-full border border-[#1e1e1e] bg-[#0a0a0a] py-3 text-xs uppercase tracking-widest text-silver transition-all hover:border-[#c9a84c] hover:text-white disabled:opacity-40">
